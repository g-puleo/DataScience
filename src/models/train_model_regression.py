...
print(df_giornoprov.columns())
'''
featuresList = ['consumo_x','T media_x', 'T media_x+1','Vento_x', 'Precipitazioni_x','Parco S. Chiara Biossido Zolfo_x', 'Parco S. Chiara Biossido di Azoto_x', 'Via Bolzano Biossido di Azoto_x', 'Rovereto Biossido di Azoto_x', 'Parco S. Chiara Biossido Zolfo_x+1', 'Vento_x+1', 'Precipitazioni_x+1', 'Parco S. Chiara Biossido di Azoto_x', 'Via Bolzano Biossido di Azoto_x', 'Rovereto Biossido di Azoto_x']
N = len(featuresList) 
ygTest = np.zeros( (N,)  ) 
ysTrain = np.zeros( (N,)  )
ygTrain = np.zeros( (N )  )
...
for jj, feats in enumerate(featuresList):
    Nfeatures[jj] = jj+1
    ygTrain[jj], ygTest[jj] = regressioneLineare(df_giornoprov, featuresList[0:jj+1], target)
    ysTrain[jj], ysTest[jj] = regressioneLineare(df_seraprov, featuresList[0:jj+1], target)
